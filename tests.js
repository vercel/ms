...
describe('performance on long strings', function () {
  describe('performance on 1k chars string', function () {
    var testVal = '1'.repeat(998) + 's'
    this.slow(75)
    this.timeout(75)
    it('should break if 100 iterations longer than 75ms', function () {
      for (var i = 0; i < 100; i++) {
        expect(ms(testVal)).to.be()
      }
    })
  })
  describe('performance on 100 chars string', function () {
    var testVal = '1'.repeat(98) + 'm'
    this.slow(50)
    this.timeout(50)
    it('should break if 100 iterations on 100 chars string longer than 50ms', function () {
      for (var i = 0; i < 100; i++) {
        expect(ms(testVal)).to.be(6.666666666666666e+101)
      }
    })
  })
})
