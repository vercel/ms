...
describe('ms(number, { long: true, roundTo: "d" | "h" | "m" | "s" })', function () {
  it('should not throw an error', function () {
    expect(function () {
      ms(500, { long: true, roundTo: 'h' });
    }).to.not.throwError();
  });

  it('should support milliseconds', function () {
    expect(ms(500, { long: true, roundTo: 'ms' })).to.be('500 ms');

    expect(ms(-500, { long: true, roundTo: 'ms' })).to.be('-500 ms');
  });

  it('should support seconds', function () {
    expect(ms(1000, { long: true, roundTo: 's' })).to.be('1 second');
    expect(ms(10000, { long: true, roundTo: 's' })).to.be('10 seconds');
    expect(ms(1200, { long: true, roundTo: 's' })).to.be('1 second');
    expect(ms(1200, { long: true, roundTo: 'ms' })).to.be('1 second 200 ms');

    // expect(ms(-1000, { long: true, roundTo: 's' })).to.be('-1 second');
    // expect(ms(-10000, { long: true, roundTo: 's' })).to.be('-10 seconds');
    // expect(ms(-1200, { long: true, roundTo: 's' })).to.be('');
    // expect(ms(-1200, { long: true, roundTo: 'ms' })).to.be('');
  });

  it('should support minutes', function () {
    expect(ms(60 * 1000, { long: true, roundTo: 'm' })).to.be('1 minute');
    expect(ms(60 * 10000, { long: true, roundTo: 'm' })).to.be('10 minutes');
    expect(ms(60 * 1000 + 2000, { long: true, roundTo: 'm' })).to.be(
      '1 minute'
    );
    expect(ms(60 * 1000 + 2000, { long: true, roundTo: 's' })).to.be(
      '1 minute 2 seconds'
    );
    expect(ms(60 * 1000 + 2000 + 200, { long: true, roundTo: 'ms' })).to.be(
      '1 minute 2 seconds 200 ms'
    );

    // expect(ms(-1 * 60 * 1000, { long: true, roundTo: 'm' })).to.be('-1 minute');
    // expect(ms(-1 * 60 * 10000, { long: true, roundTo: 'm' })).to.be('-10 minutes');
    // expect(ms(-1 * 60 * 1000 - 2000, { long: true, roundTo: 'm' })).to.be('');
    // expect(ms(-1 * 60 * 1000 - 2000, { long: true, roundTo: 's' })).to.be('');
    // expect(ms(-1 * 60 * 1000 - 2000 - 200, { long: true, roundTo: 'ms' })).to.be('');
  });

  it('should support hours', function () {
    expect(ms(60 * 60 * 1000, { long: true, roundTo: 'h' })).to.be('1 hour');
    expect(ms(60 * 60 * 10000, { long: true, roundTo: 'h' })).to.be('10 hours');
    expect(
      ms(60 * 60 * 1000 + 60 * 2000 + 2000 + 200, { long: true, roundTo: 'ms' })
    ).to.be('1 hour 2 minutes 2 seconds 200 ms');

    // expect(ms(-1 * 60 * 60 * 1000, { long: true, roundTo: 'h' })).to.be('-1 hour');
    // expect(ms(-1 * 60 * 60 * 10000, { long: true, roundTo: 'h' })).to.be('-10 hours');
    // expect(ms(-1 * 60 * 60 * 1000 - 60 * 2000 - 2000 - 200, { long: true, roundTo: 'ms' })).to.be('');
  });

  it('should support days', function () {
    expect(ms(24 * 60 * 60 * 1000, { long: true, roundTo: 'd' })).to.be(
      '1 day'
    );
    expect(ms(24 * 60 * 60 * 10000, { long: true, roundTo: 'd' })).to.be(
      '10 days'
    );
    expect(ms(24 * 60 * 60 * 1200, { long: true, roundTo: 'd' })).to.be(
      '1 day'
    );
    expect(
      ms(24 * 60 * 60 * 1000 + 60 * 60 * 2000 + 60 * 2000 + 2000 + 200, {
        long: true,
        roundTo: 'ms',
      })
    ).to.be('1 day 2 hours 2 minutes 2 seconds 200 ms');

    // expect(ms(-1 * 24 * 60 * 60 * 1000, { long: true, roundTo: 'd' })).to.be('-1 day');
    // expect(ms(-1 * 24 * 60 * 60 * 10000, { long: true, roundTo: 'd' })).to.be('-10 days');
    // expect(ms(-1 * 24 * 60 * 60 * 1000 - 60 * 60 * 2000 - 60 * 2000 - 2000 - 200, { long: true, roundTo: 'ms' })).to.be('');
  });
});
